...
For strict-input queries, you can also keep the query disabled until input exists:
...
This is useful when route params or selected values may not exist on the first render. Once the input is available, pass it normally and the query can run.

## Reusing query keys with QueryClient

React Query's `queryOptions(...)` pattern is also useful outside components. In Callsheet, the config returned by `queryOptions(...)` includes the canonical `queryKey`, so the same definition can be reused with `QueryClient`.
...
const film = useQuery(filmQuery);

queryClient.getQueryData(filmQuery.queryKey);
queryClient.invalidateQueries({ queryKey: filmQuery.queryKey });
```

This keeps imperative cache work tied to the same Callsheet-owned identity used by your hooks, instead of making you hand-write query keys separately.
