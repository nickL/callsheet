...
import { query as tsRestQuery, mutation as tsRestMutation } from '@callsheet/ts-rest';
...
export const calls = defineCalls({
  films: {
    ...
    update: mutation(UpdateFilmDocument, {
      invalidates: [['films', 'list'], ['films', 'detail']],
    }),
  },
  accounts: {
...
const account = useQuery(queryOptions(calls.accounts.byId, { input: { params: { id } } }));
...
Shared defaults like `staleTime`, and `retry` also work identically across source types. Read more in [Using Callsheet with React Query](/guides/react-query).
...
- [Manual Setup](/getting-started/manual-setup) for hand-authored calls
- [Calls](/core-concepts/calls) for the shared call model
