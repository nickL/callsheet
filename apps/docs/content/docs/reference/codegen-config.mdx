...
| `adapter`    | No       | Adapter target for generated imports. Use `'react-query'` or `'swr'`                   |
| `exportName` | No       | Name of the exported calls object. Defaults to `calls`                                 |
| `importFrom` | No       | Advanced override for the package the generated module imports Callsheet builders from |

Set exactly one of `adapter` or `importFrom`.

Use `adapter` for the normal generated workflow:
...
Use `importFrom` only when you need a custom or non-standard builder import target.
